...
async def create_normal_voting(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    text = (
        "📺 اختر القناة التي ستُنشر فيها المسابقة:\n\n"
...
@voting_router.callback_query(
    F.data.in_(["voting_setting_yes", "voting_setting_no"]), StateFilter(VotingStates)
)
...
async def vote_stars(cb: CallbackQuery) -> None:
    ...
    async for session in get_async_session():
        ...
        await repo.add_vote(
            contest_id, candidate_id, cb.from_user.id, is_star_vote=True, stars_amount=stars_amount
        )
...
async def show_voting_results(cb: CallbackQuery) -> None:
    ...
    async for session in get_async_session():
        ...
        for i, c in enumerate(
            sorted(candidates, key=lambda x: x.normal_votes + x.star_votes, reverse=True), 1
        ):
