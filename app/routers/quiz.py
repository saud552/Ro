...
from ..db.models import QuizContest, QuizQuestion, QuizParticipantScore
...
async def confirm_quiz(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    async for session in get_async_session():
        ...
        try:
            ...
            if contest.group_id:
                await cb.bot.send_message(chat_id=contest.group_id, text=text, parse_mode=ParseMode.HTML)
            else:
                await cb.bot.send_message(chat_id=contest.channel_id, text=text, parse_mode=ParseMode.HTML)
        except (TelegramBadRequest, TelegramForbiddenError):
            pass
...
async def _run_quiz_loop(bot, contest_id: int) -> None:
    ...
    async for session in get_async_session():
        ...
        questions_count = contest.question_count
