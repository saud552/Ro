...
def roulette_controls_kb(
    ...
    row1 = [InlineKeyboardButton(text="المشاركة في السحب", callback_data=f"join:{contest_id}")]
    deep_link = f"https://t.me/{bot_username}?start=notify-{contest_id}"
    ...
    if show_owner_controls:
        owner_controls = [
            InlineKeyboardButton(text="ابدأ السحب", callback_data=f"draw:{contest_id}"),
            InlineKeyboardButton(
